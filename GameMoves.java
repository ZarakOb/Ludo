...
public class GameMoves extends JPanel implements KeyListener, ActionListener,MouseListener{
    ...
    public void paint(Graphics g) {
    	...
    	if(p.pl[current_player].coin==4) {
            ...
            g.drawString("Player "+(current_player+1)+" wins.", 300, 800);
            g.drawString("Congratulations.", 300, 800);
            current_player=0;
            la = new Layout(80,50);
            p=new Build_Player(la.height,la.width);
            dice=0;
    		...
    		g.fillRoundRect(80, 550, 230,110,10,10);
    		if(current_player==0) {
				g.setColor(new Color(0xcf3c2c));
			}
			else if(current_player==1) {
				...
				g.setColor(new Color(0x0D52BD));
			}
            g.setFont(new Font("serif", Font.BOLD, 15));
            g.drawString("Player "+(current_player+1), 100, 595);
            g.drawString("Number on dice is "+dice, 100, 625);
    	}
	...
	public void mouseClicked(MouseEvent e) {
		if(flag==1) {
			...
			if(dice==6) {
				for(int i=0;i<4;i++) {
					if(p.pl[current_player].pa[i].x==x&&p.pl[current_player].pa[i].y==y&&(p.pl[current_player].pa[i].current+dice)<=56 && p.pl[current_player].pa[i].current != -1) {
						value=i;
						flag=0;
						break;
					}	
				...
				if(value!=-1) {
					...
					if(p.pl[current_player].pa[value].current==-1)
                                        {
                                            p.pl[current_player].pa[value].current = 0;
